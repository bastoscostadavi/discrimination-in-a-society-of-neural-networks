...
distrust. To compare states across parameters --- and, in principle, across
populations built from different kinds of agent --- we need statistics that depend
on neither the dimension $K$ nor the parameterisation.
...
gradients or $K$. They need only, per pair of agents, a signed alignment of opinion
and a signed trust. Any population in which those two things can be elicited ---
whatever its agents are internally --- can therefore be placed on the same diagram,
and two $N\times N$ matrices is a cheap enough measurement to run on a deployed
system. \S\ref{sec:discussion} returns to why a per-agent audit cannot substitute
for it.
