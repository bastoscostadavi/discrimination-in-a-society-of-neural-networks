...
#: Where the paper's two matched modulation figures go.  They are generated by
#: the same paired-panel routine: Figure 1 uses equal frames and no points,
#: while Figure 2 changes only the right frame and adds the measurements.
...
def main():
    ...
    # The experiments cover different regions, so the panels use independent
    # square frames. The trust panel repeats the +/-1.4 frame of trust_plane.pdf.
    # These are crops, not rescalings: the count of what falls outside is printed
    # on every run rather than left to be discovered.
    ap.add_argument("--lim", type=float, default=2.25,
                    help="opinion-panel axis range (default 2.25; 0 uses its data)")
    ...
    # Figures 1 and 2 use the same code and geometry.  Figure 1 is Figure 2 with
    # the right frame restored to the left frame and the data points removed.
    # Keeping both calls here prevents their typography, spacing, and panel size
    ...
    print(f"\n  Figures 1 and 2: alpha_w = {alpha:.3f}, alpha_mu = {alpha_mu:.3f}"
          f"   (each fitted at its own panel's clip)")
    sectors = [(F_w, r"$F_w$", h_w, h_mu, d_w, alpha),
               (F_mu, r"$F_\mu$", t_hw, t_hmu, t_d, alpha_mu)]
    plotting.figure_pair(sectors, args.paper_dir,
                         name="modulation_contours",
                         lim=args.lim or None, cap=1.6, ticks=TICKS,
                         right_lim=args.lim or None, right_ticks=TICKS,
                         show_points=False, show_panel_labels=False)
    plotting.figure_pair(sectors, args.paper_dir,
                         name="llm_modulation",
                         lim=args.lim or None, cap=1.6, ticks=TICKS,
                         right_lim=1.4, right_ticks=(-1, 0, 1),
                         show_points=True, show_panel_labels=False)
