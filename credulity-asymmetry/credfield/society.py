"""A batch of societies under a general class-dependent field.

Identical to the dynamics of the main line of work -- same agents, same update,
same agent-major layout and shared schedule -- with one thing generalized: the
field a prejudiced receiver applies is any point of the four-component basis of
:mod:`credfield.fields`, rather than one of six tabulated matrices.
...
quantities through the modulation functions of :mod:`credfield.modulation`:
...
where ``D = a + b kappa_r + c kappa_e + p kappa_r kappa_e`` for the fraction
``f`` of agents that are prejudiced, and zero for the rest.

Layout and why it matters
-------------------------
``SocietyBatch`` evolves ``R`` independent societies at once -- typically the R
pixels of a phase diagram, each with its own field and fraction.  State is
stored **agent-major**::
...
distrust, agendas and the assignment of which agents are prejudiced are all
drawn independently per run.  Pass ``shared_schedule=False`` for fully
independent schedules at ~3x the cost.
"""
...
from .fields import field_matrix
...
class SocietyBatch:
    ...
    a, b, c, p
        The four components of the field, each a scalar or an array of length
        ``R``.  See :mod:`credfield.fields`; ``p`` alone reproduces the main line
        of work's discrimination field.
    f
        Fraction of agents that are prejudiced, scalar or array of length ``R``.
    ...
    def __init__(
        ...
        # One broadcast for all five per-run parameters: passing a grid axis for
        # one of them and scalars for the rest is the normal case, and doing it
        # in one call means a length mismatch is reported once, by numpy, rather
        # than discovered later as a silently truncated sweep.
        try:
            a, b, c, p, f = np.broadcast_arrays(
                *(np.atleast_1d(np.asarray(v, dtype=dtype)) for v in (a, b, c, p, f))
            )
        except ValueError as exc:
            raise ValueError(
                "a, b, c, p and f must be scalars or arrays of one common length"
            ) from exc
        ...
        # --- who is prejudiced, and with what field -------------------------
        self.prejudiced = rng.random((N, R)) < f[None, :]
        self.D = np.zeros((N, N, R), dtype=dtype)
        for run in range(R):
            M = field_matrix(a[run], b[run], c[run], p[run])
            block = M[np.ix_(self.class_of, self.class_of)]  # (N, N)
            self.D[:, :, run] = block * self.prejudiced[:, run][:, None]
