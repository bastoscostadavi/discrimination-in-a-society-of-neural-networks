...
Two of the four field components do exactly that, and the sharpest way to see
why is that a pure ``b`` and a pure ``c`` of the same strength give trust
matrices that are *transposes* of each other -- ``s kappa_r`` against
``s kappa_e``.  A transpose leaves the symmetric part alone, so the paper's five
parameters take identical values on the two: they cannot distinguish a credulity
split from a status hierarchy, and they read zero on both.  The clean way to see it is
...
averaged over ordered pairs ``r != e``.  ``R_cred`` is the one this directory
sweeps for.  The fourth is the paper's ``R_muc``
...
two have no counterpart in the paper, and they are where a credulity asymmetry,
and its mirror a status asymmetry, live.
...
cancellation.  Under a pure credulity field the trust matrix is ``eta[r, e] =
s(kappa_r)``, one value for each *listener* class, so the symmetrized trust of a
...
vanishes however strong the split is.  Under a pure status field the same
symmetrized matrix appears -- it is the transpose, and symmetrizing forgets which
-- so the same cancellation holds and holds for the same reason.
...
*receiver* -- the first index -- so under a pure credulity field the sign of the
product is ``(-1)^(number of class-B agents in the triple)``.  Balance is
therefore decided by the *parity* of how many suspicious members a triple has,
and with equal class sizes there are exactly as many odd-parity triples as
even-parity ones, so the aggregate is exactly zero.  (Under a pure status field
each agent appears exactly once as the *emitter* instead, and the parity is the
same one.)  :func:`balance_by_composition` resolves the four cases the aggregate
averages away.
...
credulity field the credulous class is a coherent trusting bloc (``+1``) while
the suspicious class is atomized, every member distrusting every other member --
because of who is listening rather than who is speaking, but with the same
consequence -- so every triple inside it is frustrated (``-1``).
