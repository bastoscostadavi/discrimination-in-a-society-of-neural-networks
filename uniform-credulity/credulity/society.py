"""A batch of societies under a uniform credulity field.

Identical to the dynamics of the main line of work and of
``../directional-prejudice/`` -- same agents, same update, same agent-major
layout and shared schedule -- with the field specialized to its label-free
component::

    D[r, e] = a   if r is biased,   0 otherwise

This is the ``a`` direction of the four-component basis in ``dirfield.fields``:
the shift a biased receiver applies to its opinion field is the same whoever is
speaking, so it refers to no class label at all.  What it does is move the trust
separatrix.  ``h_w`` decides whether the receiver reads a message as agreement or
as disagreement, and shifting it by ``a`` shifts *how readily trust forms*,
uniformly.  ``a > 0`` is credulity, ``a < 0`` suspicion.

Because the field names no class, the 2x2 machinery a class-dependent field
needs is gone: ``D`` is one number per agent, not a block matrix.  The class
labels themselves are *kept* -- ``class_of`` and ``kappa`` are assigned exactly
as they are there, A first then B -- and are read by nothing in the dynamics.
They exist so that the class order parameters can be evaluated as a control: the
label is in the measurement and nowhere in the model, so anything they read is
finite-size noise, and that is worth being able to check rather than assume.

What *does* partition the population is which agents carry the field.  That
partition has consequences the field does not put in by hand, and resolving them
is the job of :mod:`credulity.order_params`.

Each agent is a single-layer perceptron carrying

  * an opinion sector: weight vector ``w`` (K,) and covariance ``C`` (K, K),
  * a trust sector: for every other agent, a distrust ``mu`` and its variance
    ``V``.

At each step an issue, an emitter and a receiver are drawn.  The emitter states
its opinion ``sigma_e = sign(w_e . x)``; the receiver updates all four of its
quantities through the modulation functions of :mod:`credulity.modulation`:

    gamma_C = sqrt(1 + x.C_r x)             gamma_V = sqrt(1 + V)
    h_w     = (w_r . x) sigma_e / gamma_C + D_r     h_mu = mu / gamma_V
...
pixels of a phase diagram, each with its own ``a`` and its own fraction.  State
is stored **agent-major**::

    w: (N, R, K)      C: (N, R, K, K)      mu, V: (N, N, R)

and the interaction schedule (which receiver, which emitter, which issue index)
is *shared* across the batch, so every access in the inner loop, ``C[r]`` and
``mu[r, e]``, is a contiguous view rather than a gather, and one numpy call
advances all R societies.  Sharing the schedule couples the *order* of
interactions across runs but not their content: initial weights, initial
distrust, agendas and the assignment of which agents are biased are all drawn
independently per run.  Pass ``shared_schedule=False`` for fully independent
schedules at ~3x the cost.
"""
...
#: enough interactions on the same channel.
V_FLOOR = 1e-12
...
class SocietyBatch:
    ...
    a
        Strength of the uniform field, a scalar or an array of length ``R``.
        Positive is credulity, negative suspicion.
    f
        Fraction of agents that carry it, scalar or array of length ``R``.
        Which agents those are is drawn independently per run, and independently
        of class -- there being nothing here for it to depend on.
    seed
    ...
    def __init__(
        ...
        # One broadcast for both per-run parameters: passing a grid axis for one
        # of them and a scalar for the other is the normal case, and doing it in
        # one call means a length mismatch is reported once, by numpy, rather
        # than discovered later as a silently truncated sweep.
        try:
            a, f = np.broadcast_arrays(
                *(np.atleast_1d(np.asarray(v, dtype=dtype)) for v in (a, f))
            )
        except ValueError as exc:
            raise ValueError(
                "a and f must be scalars or arrays of one common length"
            ) from exc
        ...
        # --- classes: assigned and then ignored ------------------------------
        # Split in half, A then B, exactly as in the class-dependent packages.
        # Nothing below reads these; they are here so that the class order
        # parameters can be evaluated as a control.
        ...
        # --- who is biased, and with what shift -------------------------------
        # Drawn with the same call in the same position of the stream as the
        # class-dependent packages, so a run here and a run there at the same
        # seed pick the same agents and can be compared pixel for pixel.
        self.biased = rng.random((N, R)) < f[None, :]
        #: The shift each receiver applies, ``(N, R)``.  Indexed by the receiver
        #: alone: the whole content of the uniform component is that it does not
        #: depend on the emitter, so the inner loop reads a length-R vector
        #: rather than a slice of an (N, N, R) block.
        self.D = (self.biased * a[None, :]).astype(dtype)
    ...
    def memory_bytes(self):
        return (self.w.nbytes + self.C.nbytes + self.mu.nbytes + self.V.nbytes
                + self.X.nbytes + self.D.nbytes)
    ...
    def _interact(self, r, e, x):
        ...
        # self.D[r], not self.D[r, e]: the shift is the receiver's alone.
        h_w = np.einsum("rk,rk->r", w_r, x) * sigma / gamma_C + self.D[r]
        ...
        np.maximum(V + (F_V / (gamma_V * gamma_V)) * V * V, self.v_floor,
                   out=self.V[r, e])
    ...
    def _interact_gathered(self, recv, emit, runs, x):
        ...
        h_w = (np.einsum("rk,rk->r", w_r, x) * sigma / gamma_C
               + self.D[recv, runs])
