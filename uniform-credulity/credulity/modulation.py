"""Modulation functions of the learning algorithm.

Copied unchanged from the main line of work, as it is in
``../directional-prejudice/`` too: the modulation functions belong to the
inference problem and know nothing about who is biased or about any label, so a
field cannot touch them.  A field reaches them only through the value of ``h_w``
it is handed -- which, for the uniform component, is a rigid shift of the opinion
field by ``a`` and nothing else.
...
paper highlights (and which `../nn-based-simulation/tests/test_modulation.py` checks):
