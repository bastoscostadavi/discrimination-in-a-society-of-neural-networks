...
its other components give rise to richer phase diagrams.
...
of class membership. The order parameter $\Rwmu$ alone cannot distinguish a society polarized into two camps from one
polarized into several, but here the population forms two camps as we show in Appendix~\ref{app:balance-d0} using other order parameters.

\paragraph{The agenda size effect.} The order in which
opinion and trust polarizes depends on $\alpha=P/K$ the number of issues discussed per opinion dimension. Simple agendas polarize in trust first,
while complex agendas polarize in opinion first, with a crossover near
$\alpha=1.7$ as shown in Appendix~\ref{app:balance-agenda}. In short, for small agendas you first trust and then you agree; for complex agendas you first agree and then you trust.

\paragraph{Small agenda conservation law.} Equation~\eqref{eq:update-ideo} shows
that every weight update is along $C_r\hat{x}$, which remains in the span of the agenda.
Therefore, the component
of each agent's weight vector orthogonal to that span is conserved by the dynamics.
For $\alpha<1$ each agent keeps a frozen random
component that no interaction can touch. This places a ceiling on the magnitude
of opinion overlap, so opinions cannot fully align. Trust has no corresponding
geometric constraint and can saturate at any $\alpha$. Appendix~\ref{app:conserved}
defines this bound and derives it in detail.
...
Figures~\ref{fig:phase} and \ref{fig:correlations} show the simple
agenda ($\alpha=0.17$); Appendix~\ref{app:large-agenda} shows that the complex
agenda produces qualitatively similar results.
...
\subsection{The agenda determines which comes first: distrust or disagreement}
Whether groups
dislike each other because they disagree, or disagree because they dislike each
other, remains contested in political science
\citep{fiorina2008polarization,iyengar2012affect}. Section~\ref{sec:no-disc} shows
that both orders arise in the same model. Simple agendas produce trust polarization
first, while complex agendas produce opinion polarization first. The agenda thus
determines which side of the feedback loop initiates the collective split.
...
Section~\ref{sec:model}, while Appendices~\ref{app:closed-form},
\ref{app:conserved}, and~\ref{app:derivation} provide the analytical details and
...
collective phases, so we study it in detail. Figure~\ref{fig:component-phases}
shows two other slices, with all unshown components set to zero.

The composites use four order parameters. Mean trust
$T_\mu=\langle\eta_{J|I}\rangle$ distinguishes population-wide trust from
distrust. The receiver-class correlation
$R_{\mathrm{cred}}=\langle\kappa_I\eta_{J|I}\rangle$ measures whether one class
is systematically more credulous. The difference
$A_\eta=(B_\eta^{AA}-B_\eta^{BB})/2$ measures whether the two classes have
different internal trust balance, with $B_\eta$ defined in
Equation~\eqref{eq:balance}. Finally, $\Rwmu$ from Equation~\eqref{eq:rwmu}
measures opinion--trust polarization. The $(a,f_a)$ plane encodes
$(\max[-T_\mu,0],\max[T_\mu,0],\Rwmu)$ as red, green, and blue. The $(c,f_c)$
plane instead encodes $(|R_{\mathrm{cred}}|,|A_\eta|,\Rwmu)$.
...
\section{Supplementary results on polarization}
\label{app:balance}

This appendix develops two results that support the polarization analysis in
Section~\ref{sec:no-disc}. First, the signs of relations over triples separate a
population organized into two opposing camps from one in disorder. We define
magnitude-weighted balance aggregates, report their
behavior in an unbiased population and across the $(b,\fb)$ plane, and give a
closed form that makes them cheap to compute. Second, we prove the conservation
law that explains why a simple agenda limits opinion alignment but not trust.

\subsection{An unbiased population polarizes into two camps}
...
\end{equation}
%
run over all $\binom{N}{3}$ triples \citep{alves2021frustration}. Both measures
combine sign balance with the magnitude of the pairwise relations. For independent,
symmetrically distributed random relations, their expected values are zero, but the
converse does not hold. They equal one for two internally coherent, mutually opposed
blocs only when the relations are saturated at $\pm1$. With weaker relations, the
same two-bloc sign structure gives a positive but smaller value. A negative value
means that magnitude-weighted unbalanced contributions dominate, whereas a value
near zero is ambiguous: it can reflect disorder, weak relations, or cancellation
between balanced and unbalanced triples. Like $\rho$ and $\eta$ themselves, these
measures refer to no weight, gradient, or embedding dimension.

Applied to eight independent populations of $N=400$ agents at $b=0$, trust balance
...
from the signs of the triples, rather than the magnitude-weighted value of $\Brho$:
a signed complete graph is balanced precisely when it
...
\begin{figure}[t!]
  ...
  Trajectories in the $(\Brho,\Beta)$ plane with no group bias ($b=0$), one curve
...
Figure~\ref{fig:trajectories} follows the two balance aggregates through the
$(\Brho,\Beta)$ plane as the agenda varies. This agenda dependence and the
polarization shown in Figure~\ref{fig:polarization} are also reported by
\citet{alves2021frustration}; \S\ref{sec:no-disc} gives the numbers, and the
conservation law of Appendix~\ref{app:conserved} explains why a narrow agenda
limits opinion balance.
...
\subsection{The agenda's orthogonal complement is conserved}
\label{app:conserved}
...
The simulations use the parameters listed below. We infer $K$ from the agenda
sizes reported by \citet{alves2021frustration}, and choose $\Delta t$ after
checking that the collective order parameters change little at longer times. We
choose $N$ and $P$, while $C_0$, $V_0$, and $\mu_0$ define the initial state. All
values are explicit in the code, which also reproduces the checks below.
...
\paragraph{$K$ is inferred from the reported agenda sizes.}
\citet{alves2021frustration} report balance trajectories at
...
\paragraph{$\Delta t=500$ is sufficient for the collective state.} As agents
become more certain, their updates become smaller, although the microscopic state
continues to evolve. We use $\Delta t=500$ because doubling the duration to $1000$
does not change the qualitative collective state. Across four representative
points in each phase diagram, the largest change among the five order parameters
is $0.034$ for $\alpha=0.17$ and $0.068$ for $\alpha=3.3$, with no change in phase
assignment.
...
$\fb$ and the same sign change in $b$. The differences are quantitative and follow
from the agenda-size effect described in Section~\ref{sec:no-disc}. Opinion is no
longer capped, so $\Rcw$ rises to meet $\Rmuc$ instead of lagging it, and
region~(IIIa) occupies most of the discriminatory half rather than a narrow band
at its top. $\Brho$ also saturates near its maximum across the whole $b>0$ region,
rather than remaining near its minimum.
