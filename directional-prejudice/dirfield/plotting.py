...
:func:`set_component` sets it once and :func:`add_phase_axes` reads it.
"""
...
def set_component(component):
    ...
    A sweep drives one of ``a``, ``b``, ``c``, ``p`` (:mod:`dirfield.fields`), and
...
def status_composite(r_stat, atomization, r_wmu):
    """Compose the three maps that name a state of a directional field.

    Red is the status hierarchy ``R_stat``, green the asymmetry between the two
    classes' internal cohesion, and blue the ordinary opinion-trust alignment
    ``R_wmu`` that a population reaches with no field at all.  So blue is a
    population that has polarized without reference to the label, red one sorted
    into a hierarchy, and the green channel says whether the two classes are
    differently organized internally rather than merely differently trusted.

    Both signed channels use ``|value|``: which class is on top of a hierarchy is
    a matter of the sign of the field, and mirroring the colour with it would
    make half of every map read as empty when it is fully ordered.  The sign is
    read off the individual maps, which keep it.
    """
    def norm(a, key):
        lo, hi = RANGES[key]
        return np.clip((a - lo) / (hi - lo), 0.0, 1.0)

    return np.stack([norm(np.abs(r_stat), "R_wmu"),
